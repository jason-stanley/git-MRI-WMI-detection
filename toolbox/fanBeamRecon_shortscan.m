function img = fanBeamRecon_shortscan_wpw(SID,SAD,dx,dy,ReconSize,sino,py,DetectorSize,shift_c,ang,incre,parkerWeighting)
...
% parkerWeighting = 1;
if parkerWeighting==1
    Num_of_proj_total = length(ang)-1;
    Num_of_proj = length(ang)-1;
else
    Num_of_proj_total = length(ang);
    Num_of_proj = length(ang);
end;
...
H = SAD/SID*delta_Y*designFilter2(delta_Y,'hamming',len,1.0)';
...
%     end
    ...
    if parkerWeighting;
        ...
        parkerWeight = zeros(size(proj));
%         beta = abs((i-start_inx)*Angle_incre);
        beta = abs((param(i+1,1)-param(1,1))*pi/180);
        temp_1 = find((delta-alpha)>=beta/2);
        parkerWeight(temp_1) = sin(pi/4*beta./(delta-alpha(temp_1))).^2;
        temp_2 = find(((pi-2*alpha)<=beta)&((pi+2*delta)>=beta));
        parkerWeight(temp_2) = sin(pi/4*(pi+2*delta-beta)./(alpha(temp_2)+delta)).^2;
        temp_3 = find(((delta-alpha)<=beta/2)&((pi-2*alpha)>=beta));
        parkerWeight(temp_3) = ones([length(temp_3) 1]);
        
        proj = proj.*parkerWeight;
    end
...
%     delta_phi = abs(param(2)-param(1))*pi/180;
%     img_temp = delta_phi*img_temp;
    img = img+img_temp;

end
% 
% % weight the final recon volume.
if parkerWeighting;
    img = abs(ang(2)-ang(1))*incre*img;
else
    img = pi/(Num_of_proj/incre)*img;
end
